...
The service/tool layering is designed so write tools slot in without rewiring: `git_branch_create`, `git_stage`, `git_unstage`, `git_commit`, `git_push`, `git_force_push`, `git_checkout`, `git_reset`, `git_merge`, `github_pr_merge`, `github_pr_close`, `github_pr_comment`, `github_pr_review`. The mutation phase has started with `github_pr_create` (creates a PR via `gh pr create`; never touches local history). Later phases may add riskier tools (force-push, reset, merge) — each lands as its own tool with its blast radius stated in the description, structured errors, and safety rails appropriate to what it can destroy (e.g. `--force-with-lease` over `--force`, dry-run flags where available). No mutation is ever fired by automatic sampling or refresh paths; all writes ride an explicit user action or agent turn.
