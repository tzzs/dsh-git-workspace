...
export function apply(ctx:{tools:{register(tool:unknown):unknown}}){
 const add=(name:string,description:string,parameters:Record<string,unknown>,execute:(a:Record<string,unknown>)=>Promise<unknown>)=>ctx.tools.register(defineTool({name,description,parameters: parameters as never,output:{schema:json,render:(_args: never,_value: never)=>[{type:'text',text:'Git workspace result'}]},async execute(args: Record<string, unknown>){const result=await execute(args as Record<string,unknown>);return JSON.parse(JSON.stringify(result)) as never}} as never))
