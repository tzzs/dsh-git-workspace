...
- `make integration` / `make agent-loop` runs `scripts/agent-loop-integration.mjs`. The script resolves the globally installed `@deepseek-ai/dsh` package at runtime via `npm root -g` (override with `DSH_GLOBAL_ROOT`), so it works on any machine with `npm i -g @deepseek-ai/dsh`; it fails with a clear message when the package cannot be found. `tests/harness-runtime.test.js` remains the dependency-free substitute.
- Tests import compiled output from `lib/`, never from `src/`. `tests/lib-fresh.test.js` guards against a stale or missing `lib/`: running `node --test tests/*.test.js` directly without a prior build fails fast with a clear message instead of confusing errors deep inside other tests. `npm test` builds first, so it always passes the guard.
- `make local-install` / `local-config` / `local-run` verify the plugin inside an isolated dsh profile using a corepack pnpm shim (`.dsh-bin/pnpm`). Requires the `dsh` CLI on `PATH`. In the Makefile, `$DSH_HOME` defaults to `~/.dsh-git-workspace`, but the `dsh` CLI itself defaults to `~/.dsh` when `DSH_HOME` is unset — a profile installed under one home is invisible under the other. All `local-*` targets now fail loud via `check-profile-includes-plugin` (dump-config must list the plugin) instead of silently booting without it, and `check-dsh-home` rejects `/mnt/*|/media/*` DSH_HOME paths up front (WSL drvfs: dsh's `dsh-credentials-local` enforces `chmod 600` on its credentials file, impossible on NTFS-backed mounts). `make local-run` is **headless** (bundle is only `@deepseek-ai/dsh-base`, so it prints no port — that's expected, not a hang). To verify in the browser UI, use `make local-web` (creates `$DSH_HOME/profiles/web` with `dsh-web-app` and serves on a free port); the URL banner appears only after the loader settles (~10–15s).
